...
- [x] First-run pairing wizard (CLI) — **DONE 2026-05-20.** `pair_setup.py:87` wizard enumerates paired devices, trusts, writes `local.env`, walks iPhone toggles (Show Message Notifications / Sync Contacts) with CoD sudoers hint.
...
- [x] **ANCS** for per-app notifications — **DONE 2026-08-29.** `ancs/client.py:1` already probes ANCS UUIDs via `ObjectManager.InterfacesAdded` + `GetManagedObjects` instead of `ServicesResolved` (matching bmh129 `0db80f3`), plus `systemd/set-le-bearer.sh:1` + `install-ancs-sudoers.sh:1` handle `LastUsedBearer=le` for BLE bias. Added `tests/test_ancs_parsers.py:1` (13 tests) for wire-format parsers/builders.
